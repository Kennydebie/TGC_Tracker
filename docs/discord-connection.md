...
Status: the integration has both a Cloudflare Durable Object listener and a
Docker fallback. A real bot token, server installation and one deployed
listener are still required. Tests with fixtures do not establish a live
Discord connection.
...
listener has no public route and exactly one named Durable Object owns the
Gateway session, preventing duplicate bot connections.

Alternatively, on an always-on Docker host, place the worker's three settings
in an ignored `.env.discord`, then run:
...
The Docker named state volume retains pending messages across restarts, with
maximum 24-hour raw retention and a 2,000-event queue cap. Run one Docker
replica. No port needs exposing. The ordinary Sites web Worker cannot run the
Gateway permanently; the Durable Object or Docker listener is a separate
deployment.
...
- Discord Gateway READY/RESUMED establish connection. Heartbeat ACK failures
  trigger reconnect/resume with backoff. The Durable Object persists session
  and sequence state across restarts and uses an alarm plus Cron watchdog to
  recover from eviction or deployment restarts. Fatal authentication/intent
  errors retry only after a long cool-down.
